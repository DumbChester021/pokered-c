...
For more details on the C conversion architecture, expansion mechanics, and critical constraints (**Bank Safety** and Struct Alignment), see [**docs/MIGRATION_GUIDE.md**](docs/MIGRATION_GUIDE.md) and the [**Technical Research Report**](docs/RESEARCH_REPORT.md).

**Stability Warnings:**
- **Bank Safety**: When writing C logic, ALWAYS use the **Bank-Safe Bridge** (`far_call.h`) for any cross-bank function calls or data reads. Direct C pointers across banks WILL cause memory corruption.
- **Logic Parity**: All migrated C routines are verified via **Dual-Execution Testing** against original ASM to ensure identical register/WRAM mutation state.
